...
class CurrencyOperationsLoadTest {
    ...
    private companion object {
        const val MAX_CONVERSION_TIME_MS = 5000L
        const val MAX_CACHE_OPERATION_TIME_MS = 100L
        const val MAX_CONCURRENT_OPERATION_TIME_MS = 10000L
        const val MAX_MEMORY_OPERATION_TIME_MS = 3000L
        const val MAX_STRESS_TEST_TIME_MS = 15000L
    ...
    fun loadTestHighFrequencyConversions() {
        ...
        benchmarkRule.measureRepeated {
            ...
            runBlocking {
                val conversions = (1..100).map { index ->
                    async {
    ...
    fun loadTestLargeDatasetOperations() {
        ...
        benchmarkRule.measureRepeated {
            ...
            runBlocking {
                val operations = amounts.take(200).map { amount ->
                    async {
    ...
    fun loadTestConcurrentOperations() {
        ...
        benchmarkRule.measureRepeated {
            ...
            runBlocking {
                val concurrentOperations = (1..30).map { index ->
                    async {
                ...
                assertTrue("Should have 30 concurrent operation groups", results.size == 30)
    ...
    fun loadTestStressTesting() {
        ...
        benchmarkRule.measureRepeated {
            ...
            runBlocking {
                ...
                val stressOperations = (1..100).map { index ->
                    async {
                        try {
                            withTimeout(MAX_CONVERSION_TIME_MS) {
                                ...
                                val fromCurrency = currencies.random().code
                                val toCurrency = currencies.random().code
                                val amount = (100.0..10000.0).random().toDouble()
